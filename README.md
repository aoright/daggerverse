...
$ dagger install github.com/typesafe-ai/daggerverse/ruff
...
| [`ruff`](./ruff) | Lint and format Python code using [Ruff](https://docs.astral.sh/ruff/). Exposes checker (`ruff check`) and formatter (`ruff format`) sub-objects with `lint` checks, auto-detection of `ruff` versions from `uv.lock`/`ruff.toml`/`pyproject.toml`, and auto-fix functions returning a `Changeset`. |
